#!/usr/bin/env python3
"""
4-edge transistor CLLN transfer-sweep script.
...
Notes:
    ...
    - The loaded hidden node is CLI-controllable with --ground-node and defaults to h1.
    - Using a GOhm/TOhm load would effectively open the network and make the transfer
      nearly trivial.
    - The full 5^4 sweep is 625 gate combinations. With 20 Vin points, that is
      12,500 operating-point solves. Use --max-combos for smoke tests.
"""
...
def mk_switch_netlist(
    ...
    lines: List[str] = []
    lines.append(".title four_edge_transfer_sweep_ald1106")
...
def _parse_args():
    p = argparse.ArgumentParser(description="4-edge transistor CLLN transfer sweep")
    ...
    p.add_argument("--ground-node", type=str, choices=["h1", "h2"], default="h1")
    ...
    p.add_argument("--shuffle-combos", action="store_true", help="Shuffle gate-combination order before running")
...
def main() -> None:
    ...
    if env_run_dir:
        ...
        run_id = datetime.now().strftime("%Y%m%d-%H%M%S") + f"_four_edge_transfer_seed-{args.seed}"
    ...
    meta = {
        ...
        "variant": "four_edge_transfer_sweep_ald1106",
        "backend": "shared",
        "solver": str(args.solver).lower(),
        "seed": int(args.seed),
        "topology": {
        ...
        "gate_grid": gate_grid,
        "shuffle_combos": bool(args.shuffle_combos),
    ...
    print("=== RUN START (four_edge_transfer_sweep_ald1106) ===", flush=True)
    print(f"Gate grid: {gate_grid}", flush=True)
    print(f"Shuffle combinations: {bool(args.shuffle_combos)}", flush=True)
    ...
    combo_list = list(itertools.product(gate_grid, repeat=n_edges))
    ...
    with open(csv_path, "w", newline="") as fcsv:
        ...
        for combo_idx, gates in enumerate(combo_list):
    ...
    print("=== RUN END (four_edge_transfer_sweep_ald1106) ===", flush=True)
