#!/usr/bin/env python3
"""
Linear-MLP baseline for the 16-token language task.

Model:
  z = W x + b   (24 -> 16, no hidden nonlinearity)
  p = softmax(z / T)

Loss:
  Soft cross-entropy against context-conditioned empirical target q(.|x),
  where q is built from the TRAIN split exactly like clln_lang_trainer_ce.py.
"""
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Linear-MLP 24->16 baseline, soft CE")
    ...
    p.add_argument("--template-mode", type=str, choices=["tiny", "balanced"], default="balanced")
...
def eval_metrics(
    ...
    q = build_q_matrix(ctx_list, q_map_train, unigram_q_train)
    ...
    return EvalOut(exact_acc=exact_acc, support_acc=support_acc, soft_ce=ce, qmass_mean=qmass)
...
def main() -> None:
    ...
    all_x, all_y, all_ctx_keys, _ = build_language_dataset(
    ...
    qtr = build_q_matrix(ctr, q_map_train, unigram_q_train)

    rng = np.random.default_rng(seed)
    W = rng.normal(0.0, float(args.init_std), size=(OUTPUT_DIM, INPUT_DIM)).astype(float)
    b = np.zeros((OUTPUT_DIM,), dtype=float)

    results_dir = Path(__file__).resolve().parent / "results_language_16_linear_softce"
    ...
    for ep in range(1, int(args.epochs) + 1):
        ...
        print(
            f"[epoch {ep}/{args.epochs}] lr={lr} T={temp} "
            f"| TRAIN exact={tr.exact_acc:.4f} support={tr.support_acc:.4f} ce={tr.soft_ce:.4f} qmass={tr.qmass_mean:.4f} "
            f"| VAL exact={va.exact_acc:.4f} support={va.support_acc:.4f} ce={va.soft_ce:.4f} qmass={va.qmass_mean:.4f}",
            flush=True,
        )
