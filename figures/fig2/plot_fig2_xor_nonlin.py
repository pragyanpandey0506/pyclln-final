#!/usr/bin/env python3
"""
Figure 2: 2x2 single-column panel with:
  - XOR 4x4 MSE
  - XOR 4x4 accuracy
  - nonlinear-regression MSE
  - nonlinear-regression fit snapshots

The output bundle lives beside this script under figures/fig2/.
"""
...
DEFAULT_NONLIN_RUN = ROOT / "nonlin_reg" / "results" / "pnas_4x4_gamma0p4_truepts_25000ep"
DEFAULT_OUT_PREFIX = Path(__file__).resolve().parent / "fig2_xor_nonlin_2x2_singlecol"
DEFAULT_SNAPSHOTS = [0, 100, 1000, 10000, 25000]
DEFAULT_XOR_INCLUDE_RUNS = ["seed-3"]


def _parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build the Figure 2 XOR/nonlinear 2x2 panel")
    ...
    p.add_argument("--width-in", type=float, default=3.5)
    p.add_argument("--height-in", type=float, default=5.2)
    return p.parse_args()
...
def main() -> None:
    ...
    fig.subplots_adjust(left=0.16, right=0.987, bottom=0.10, top=0.935, wspace=0.42, hspace=0.28)
    ...
    ax = axes[1, 0]
    ax.plot(np.arange(nonlin_mse.size), np.clip(nonlin_mse, np.finfo(float).tiny, None), color="black", linewidth=1.1)
    ax.set_xlabel("Epoch")
    ax.set_ylabel("MSE")
    ax.set_yscale("log")
