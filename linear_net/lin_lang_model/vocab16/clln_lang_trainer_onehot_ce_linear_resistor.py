#!/usr/bin/env python3
"""
Linear-resistor KCL variant of the 16-token one-hot CE language trainer.
...
replaces ngspice with an analytic nodal solve for a dense linear resistor
network.
...
Circuit model:
  ...
  - Output clamp uses the same series-resistor-to-target-voltage construction
    as the analog trainer.

Because the topology has no hidden nodes, each phase solve is just the KCL
solution of 16 independent output nodes:
...
def parse_args():
    p = argparse.ArgumentParser(
        description="CLLN dense 24->16 language trainer, one-hot CE, linear resistor KCL backend"
    )
...
def build_linear_manifest_text(
    *,
    ...
    return (
        ".title clln_dense_language16_onehotxent_linear_resistor\n"
...
def run_controller(args: argparse.Namespace) -> None:
    ...
    results_dir = Path(__file__).resolve().parent / "results_language_16_linear_resistor_onehotce"
    ...
    print("=== RUN START (clln_dense_language16_onehotxent_linear_resistor) ===", flush=True)
    ...
    print("=== RUN END (clln_dense_language16_onehotxent_linear_resistor) ===", flush=True)
    try:
        log_f.flush()
        log_f.close()
    except Exception:
